...
def mock_pypi_request_fetch_page():
    library_html_page_response = open(
        "tests/mock_pypi_library_page_response.html", "r", encoding="utf-8"
    ).read()
    mock_get_request = mock.Mock(
        side_effect=lambda k: {}.get(k, mock.Mock(text=library_html_page_response))
    )
...
def requirements_input_file():
    ...
    with mock.patch(
        "pypisd.main.argparse.ArgumentParser.parse_args",
        return_value=argparse.Namespace(
            input_file=requirements_file_path, output_file="pypi_sd_links_from_file.csv"
        ),
    ):
        yield mock
...
def toml_input_file():
    ...
    with mock.patch(
        "pypisd.main.argparse.ArgumentParser.parse_args",
        return_value=argparse.Namespace(
            input_file=toml_file_path, output_file="pypi_sd_links_from_file.csv"
        ),
    ):
        yield mock
